...
import { History, Edit, Trash2, Sun, Moon, Clock } from 'lucide-react';
...
export default function TripHistoryPanel() {
  ...
  const [selectedPendingEntry, setSelectedPendingEntry] = useState<OtherPending | null>(null);
  ...
  const sortedPendingInRange = [...pendingInRange].sort((a, b) => a.date.localeCompare(b.date));
  ...
  const totalPendingAmount = sortedPendingInRange.reduce((sum, p) => sum + p.amount, 0);
  ...
  const editCoTravellerEntry: CoTravellerIncomeEntry | null =
    selectedEntry && selectedEntry.type === 'coTraveller' && selectedEntry.coTravellerIncomeId
      ? (() => {
          const income = coTravellerIncomes.find((i) => i.id === selectedEntry.coTravellerIncomeId);
          return income
            ? {
                id: income.id,
                amount: income.amount,
  ...
  const handleSaveTravellerEdit = (updated: TripHistoryEntry) => {
    updateTravellerTrip(updated.dateKey, updated.travellerId, updated.morning, updated.evening);
    toast.success('Trip entry updated successfully');
    setEditTravellerDialogOpen(false);
    setSelectedEntry(null);
  };

  const handleSaveCoTravellerEdit = (entry: Omit<CoTravellerIncomeEntry, 'id'>) => {
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>All trips and pending amounts within the selected date range</CardDescription>
        </CardHeader>
                            ...
                            <div className="flex items-center justify-center gap-1">
                              {entry.tripTime === 'morning' ? (
                                <>
                                  ...
                                  <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Morning</span>
                                </>
                              ) : (
                                <>
                                  <Moon className="h-3.5 w-3.5 text-indigo-400" />
                                  <span className="text-xs font-medium text-indigo-500 dark:text-indigo-400">Evening</span>
                                </>
                              )}
                            </div>
                          ) : (
                      ...
                      return (
                        <TableRow key={pending.id}>
                          ...
                          <TableCell className="text-muted-foreground text-sm">
                            {pending.note || <span className="italic text-muted-foreground/60">—</span>}
                          </TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button

...
export function useLedgerLocalState() {
  ...
  useEffect(() => { travellersRef.current = travellers; }, [travellers]);
  useEffect(() => { dailyDataRef.current = dailyData; }, [dailyData]);
  useEffect(() => { dateRangeRef.current = dateRange; }, [dateRange]);
  useEffect(() => { ratePerTripRef.current = ratePerTrip; }, [ratePerTrip]);
  useEffect(() => { cashPaymentsRef.current = cashPayments; }, [cashPayments]);
  useEffect(() => { otherPendingRef.current = otherPending; }, [otherPending]);
  useEffect(() => { carExpensesRef.current = carExpenses; }, [carExpenses]);
  useEffect(() => { includeSaturdayRef.current = includeSaturday; }, [includeSaturday]);
  useEffect(() => { includeSundayRef.current = includeSunday; }, [includeSunday]);
  useEffect(() => { coTravellerIncomesRef.current = coTravellerIncomes; }, [coTravellerIncomes]);
  useEffect(() => { perDayAutoTollSelectionRef.current = perDayAutoTollSelection; }, [perDayAutoTollSelection]);

  // Build a snapshot of the current state for immediate persistence
  const buildSnapshot = (overrides: Partial<StoredState> = {}): StoredState => ({
  ...
  const updateOtherPending = (id: string, updates: Partial<Omit<OtherPending, 'id'>>) => {
  ...
  const mergeRestoreFromBackup = (backupState: LocalLedgerState) => {
    ...
    setPerDayAutoTollSelection((merged.perDayAutoTollSelection as PerDayAutoTollSelection) ?? {});
  };
